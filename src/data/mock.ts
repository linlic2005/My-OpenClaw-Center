  ...
  return [
    {
      ...
      summary: language === "zh-CN" ? "梳理 OpenClaw 客户端范围" : "Reviewing the OpenClaw client scope",
      updatedAt: baseTime - 1000 * 60 * 5
    },
    {
      id: "sess_review",
      name: language === "zh-CN" ? "代码审查" : "Code Review",
      summary: language === "zh-CN" ? "检查认证流程和文件上传链路" : "Inspecting auth flow and file upload pipeline",
      updatedAt: baseTime - 1000 * 60 * 55
    },
    {
      id: "sess_ops",
      name: language === "zh-CN" ? "运维同步" : "Ops Sync",
      summary: language === "zh-CN" ? "确认 Gateway 连通性和健康状态" : "Checking gateway reachability and health",
    ...
    sess_project: [
      {
        ...
        content:
          language === "zh-CN"
            ? "你好，请帮我梳理一下这个客户端当前最重要的开发优先级。"
            : "Hi, please help me outline the top development priorities for this client.",
...
export function getMockColumns(language: AppLanguage): KanbanColumn[] {
  return [
    ...
    { id: "doing", title: language === "zh-CN" ? "进行中" : "In Progress", color: "#3182ce", order: 1 },
    { id: "done", title: language === "zh-CN" ? "已完成" : "Done", color: "#2f855a", order: 2 }
  ];
}
  ...
  return [
    {
      id: "card_shell",
      columnId: "todo",
      title: language === "zh-CN" ? "搭建 Tauri 项目骨架" : "Build the Tauri project foundation",
      description:
        language === "zh-CN"
          ? "初始化前端目录、服务层、状态层和 Rust 命令。"
          : "Initialize frontend folders, services, stores, and Rust commands.",
      order: 0,
      labels: [{ id: "core", name: language === "zh-CN" ? "核心" : "Core", color: "#c53030" }],
      comments: 2,
      syncStatus: "idle",
      dueDate: "2026-04-02"
    },
    ...
    },
    {
      id: "card_files",
      columnId: "done",
      title: language === "zh-CN" ? "定义文件 API 协议" : "Define the file API contract",
      description:
        language === "zh-CN"
          ? "分片上传、下载初始化和目录列举协议已整理。"
          : "Chunked upload, download init, and directory listing contracts are drafted.",
      order: 0,
      labels: [{ id: "api", name: "API", color: "#2f855a" }],
      comments: 1,
      syncStatus: "idle"
    }
  ];
...
export function getMockFiles(language: AppLanguage): FileItem[] {
  return [
    ...
    },
    {
      ...
      content:
        language === "zh-CN"
          ? "# OpenClaw Center\n\n一个基于 React + Tauri 的远程协作客户端。"
          : "# OpenClaw Center\n\nA remote collaboration client built with React and Tauri."
    },
    {
      id: "file_app",
      name: "App.tsx",
